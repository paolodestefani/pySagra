...
class VariantCheckBox(QCheckBox):
    ...
    def __init__(self, parent: Optional[QWidget], desc: str, priced: Decimal) -> None:
        super().__init__(parent)
        self.priceDelta: Decimal = priced
        if priced > 0:
...
class ChooseVariantDialog(QDialog):
    ...
    def __init__(self, parent: QWidget, item: str, variants: list) -> None:
        ...
        for variant, delta in variants:
            ...
            v = VariantCheckBox(None, variant, delta)  
            self.bg.addButton(v)
            self.ui.layout.addWidget(v)

    def getVariants(self) -> tuple[str, Decimal]:
...
class BaseOrderDialog(QDialog):
    ...
    def resetDialog(self) -> None:
        ...
        with gui_exception_context(self, _tr('OrderDialog', "Loading items from database")):
            for dept_id, dept_name in department_list(include_menu=True):
                ...
                for item_id, item_desc, item_price, row_pos, col_pos, has_stock, stock_val, txt_color, bg_color, has_vars, current_lvl in item_list(session['event_id'], dept_id):
                    ...
                    btn.id = item_id
                    btn.price = item_price
                    btn.sc = has_stock
                    btn.hasVariants = has_vars
                    btn.level = current_lvl
            ...
            if not ivars:
                if (not self.ui.pushButtonVariants.isEnabled()) or self.ui.pushButtonVariants.isChecked():
                    ...
                    if rv:
                        ...
                        priced = Decimal(str(variant_price or 0.0))
                    dlg.deleteLater()  
                    if not rv:
                        return
            ...
            if order_item_id and order_item_vars:
                ...
                if same_id and same_vars:
                    ...
                    if qty_item and price_item and amount_item:
                        ...
                        # Recalculate amount using Decimal precision
                        raw_data = price_item.data(Qt.ItemDataRole.DisplayRole) # becausefromCurrency expects a string input
                        price_val = fromCurrency(str(raw_data) if raw_data is not None else '0.0')
                        price_decimal = Decimal(str(price_val))
                        qty_decimal = Decimal(old_qty + qty)
    ...
    def orderCellClicked(self, row: int, column: int) -> None:
        ...
        # Clean string cast for fromCurrency to satisfy mypy
        raw_price_data = price_item.data(Qt.ItemDataRole.DisplayRole)
        price = fromCurrency(str(raw_price_data) if raw_price_data is not None else '0.0')
        # Ensure Decimal calculations for the new amount row
        price_decimal = Decimal(str(price))
        amount_item.setData(Qt.ItemDataRole.DisplayRole, toCurrency(Decimal(new_qty) * price_decimal))
        self.recalcTotals()
    ...
    def accept(self) -> None:
        ...
        subtotal_check = Decimal(str(self.ui.doubleSpinBoxSubTotal.value()))
        discount_check = Decimal(str(self.ui.doubleSpinBoxDiscount.value()))
        ...
        for i in range(self.ui.tabWidgetOrder.rowCount()):
            ...
            if id_cell and vars_cell and qty_cell and price_cell and amount_cell:
                ...
                line['quantity'] = int(qty_cell.data(Qt.ItemDataRole.DisplayRole))
                ...
                line['price'] = fromCurrency(str(raw_price) if raw_price is not None else '0.0')
                line['amount'] = fromCurrency(str(raw_amount) if raw_amount is not None else '0.0')
